...
void upgradeInitCommand(client *c) {
    if (server.upgrade_drained) {
        addReplyError(c, "UPGRADE already completed (data drained). Restart node to serve another UPGRADE.");
        return;
    }
...
void upgradeChannelCommand(client *c) {
    ...
    if (rs->channels_registered == rs->total_threads) {
        ...
        /* Clear expires and volatile-items kvstores for all dbs — their entries
         * are shared pointers with db->keys and will be freed during the drain.
         * Both types have NULL destructor so this just clears the structures. */
        for (int dbid = 0; dbid < server.dbnum; dbid++) {
            if (server.db[dbid] != NULL) {
                kvstoreEmpty(server.db[dbid]->expires, NULL);
                kvstoreEmpty(server.db[dbid]->keys_with_volatile_items, NULL);
            }
        }
...
void upgradeCron(void) {
    ...
    if (rs->sending) {
        ...
        /* Mark keys hashtables as empty. The objects were already freed by
         * the sender threads (in-place upgrade). Set used=0 so the hashtable
         * is logically empty; bucket memory stays allocated until process exit. */
        for (int dbid = 0; dbid < server.dbnum; dbid++) {
            if (server.db[dbid] == NULL) continue;
            hashtable *ht = kvstoreGetHashtable(server.db[dbid]->keys, 0);
            if (ht != NULL) hashtableSetUsedCount(ht, 0);
        }
        server.upgrade_drained = 1;

        /* Re-enable rehashing */
        server.dict_resizing = 1;
        hashtableSetResizePolicy(HASHTABLE_RESIZE_ALLOW);

        serverLog(LL_NOTICE, "UPGRADE.CHANNEL: bulk transfer complete (in-place). %lld keys, %lld bytes%s. Entering delta phase.",
